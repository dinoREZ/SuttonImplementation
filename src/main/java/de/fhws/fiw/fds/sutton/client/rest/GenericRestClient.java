...
public class GenericRestClient<M extends AbstractClientModel> {
	...
	public GenericRestClient(final HeaderMap headers) {
		this.webClient = new GenericWebClient(ClientUser.ANONYMOUS.getUserName(),
				ClientUser.ANONYMOUS.getPassword());
	}
	...
	public RestApiResponse<M> sendGetSingleRequest(final String url, final Class<M> clazz)
			throws IOException {
		return toGetSingleResourceRestApiResponse(this.webClient.sendGetSingleRequest(url, clazz));
	}

	private RestApiResponse<M> toGetSingleResourceRestApiResponse(
			final WebApiResponse<M> webApiResponse) {
		final Optional<M> singleResourceOptional =
				webApiResponse.getResponseData().stream().findFirst();
		...
		return new RestApiResponse<>(webApiResponse.getLastStatusCode(), singleResource,
				webApiResponse.getResponseHeaders(), parseLinkHeaders(webApiResponse));
	}
	...
	private Map<String, Link> parseLinkHeaders(final WebApiResponse<M> webApiResponse) {
		...
		final Collection<Link> parsedLinkHeaders =
				linkHeaders.stream().map(Link::parseFromHttpHeader).collect(Collectors.toList());

		return parsedLinkHeaders.stream().collect(Collectors.toMap(Link::getRelationType, h -> h));
	}
	...
	public RestApiResponse<M> sendGetCollectionRequest(final String url,
		...
		return toGetCollectionResourceRestApiResponse(
				this.webClient.sendGetCollectionRequest(url, genericType));
	}

	private RestApiResponse<M> toGetCollectionResourceRestApiResponse(
			final WebApiResponse<M> webApiResponse) {
		return new RestApiResponse<>(webApiResponse.getLastStatusCode(),
				webApiResponse.getResponseData(), webApiResponse.getResponseHeaders(),
	...
	private RestApiResponse<M> toPostResourceRestApiResponse(
			final WebApiResponse<M> webApiResponse) {
		return new RestApiResponse<>(webApiResponse.getLastStatusCode(),
